...
def setup_target():
    ...
    # 1. Connect to Docker
    api_socket = os.environ.get("DOCKER_HOST")
    if not api_socket and "colima" in os.environ.get("HOME", ""):
        # Fallback heuristic for this user environment if not set
        username = os.environ.get("USER", "sanidhyakumar")
        api_socket = f"unix:///Users/{username}/.colima/default/docker.sock"
        os.environ["DOCKER_HOST"] = api_socket
    ...
    with conn.cursor() as cur:
        ...
        # Create Table
        cur.execute("CREATE TABLE IF NOT EXISTS users (id serial PRIMARY KEY, email text, created_at timestamp DEFAULT now())")
