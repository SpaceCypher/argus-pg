...
async def test_validate_improvement():
    ...
    definition = IndexDefinition(
        table_name="big_table", columns=["val"], method="btree"
    )
    suggestion = IndexSuggestion(
        target_query_id="int_q1", definition=definition, reasoning="Integration Test"
    )
    ...
    print(
        f"Integration Check: Baseline={res.validation.original_cost} -> New={res.validation.new_cost}"
    )
